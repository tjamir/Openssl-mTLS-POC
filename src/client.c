...
int create_socket(char *address, int port)
{
    ...
    connect_result = connect(socket_fd, &server_address, sizeof(server_address));
...
void init_client_connection(int socket_fd, ssl_client_connection *connection, SSL_CTX *ssl_ctx, void (*client)(ssl_client_connection))
{
