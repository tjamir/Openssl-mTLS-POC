...
typedef struct ssl_server_connection
{
    int client;
    SSL *ssl;
    void (*service)(struct ssl_server_connection *);
...
void echo_service(ssl_server_connection *connection)
{
    const length = 16 * 1024;
    char buffer[length];
    int read = SSL_read(connection->ssl, buffer, length);

    printf("Message from client: %s\n", buffer);

    if (read > 0)
    {
        SSL_write(connection->ssl, buffer, strlen(buffer));
    }
}
...
void init_server_connection(ssl_server_connection *connection, SSL_CTX *ctx, int client, void (*service)(ssl_server_connection *))
{
